...
public class Solution {
    ...
    public static boolean Find(int target, int[][] array) {
        ...
        int rowHigh = rowLen - 1;
        int colHigh = colLen - 1;
        return Find1(target, array, 0, 0, rowHigh, colHigh);
    }

    public static boolean Find1(int target, int[][] array, int rowLow, int colLow, int rowHigh, int colHigh) {
        if (rowLow > rowHigh || colLow > colHigh) {
            return false;
        }

        if (target < array[rowLow][colLow] || target > array[rowHigh][colHigh]) {
            return false;
        }

        int midRow = rowLow + ((rowHigh - rowLow) >> 1);
        int midCol = colLow + ((colHigh - colLow) >> 1);

        if (array[midRow][midCol] < target) { //2,3,4
            //2
            return Find1(target, array, rowLow, midCol + 1, midRow, colHigh) ||
                       //3
                       Find1(target, array, midRow + 1, colLow, rowHigh, midCol) ||
                       // 4
                       Find1(target, array, midRow + 1, midCol + 1, rowHigh, colHigh);

        } else if (array[midRow][midCol] > target) { //1,2,3
            // 1
            return Find1(target, array, rowLow, colLow, midRow, midCol) ||
                       //2
                       Find1(target, array, rowLow, midCol + 1, midRow, colHigh) ||
                       //3
                       Find1(target, array, midRow + 1, colLow, rowHigh, midCol);
        } else {
            return true;
